...
class Presentations extends React.Component {
  ...
  render() {
    const { items, isFetching, nextPage, initialItems } = this.props
    const { itemsNumber } = this.state
    if (initialItems === itemsNumber && isFetching) {
      return <Loading />
    }
    return (
      <>
        <div className="columns is-multiline">
          {items.map(item => (
            <PresentationCard item={item} key={item.presentation_link} />
          ))}
        </div>
