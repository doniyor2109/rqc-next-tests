...
const Publication = props => {
  ...
  const resultInTitle =
    searchRequest && simpleSearch(item.data.title[0], searchRequest, false)
  const resultInJournal =
    searchRequest &&
  ...
  return (
    <Result search={!searchRequest}>
        ...
        <h1>
          {resultInTitle
            ? resultInTitle.map((res, index) => (
                <span
                  key={res + Math.random().toString(36)}
                  className={index === 1 ? 'bold' : undefined}
                >
                  {res}
                </span>
              ))
      ...
      <Journal>
        <JournalName
          href={Link.url(item.data.journal_url, PrismicConfig.linkResolver)}
        >
          {resultInJournal
        ...
        {item.data.pages.length > 0 && (
          <Pages>pp.: {item.data.pages[0] && item.data.pages[0].text}</Pages>
        )}
      </Journal>
      {item.data.eprint.length > 0 && (
        <Arxiv
          href={`https://arxiv.org/pdf/${item.data.eprint[0].text}`}
          target="_blank"
          rel="noopener noreferer"
        >
          Arxiv: {item.data.eprint[0] && item.data.eprint[0].text}
        </Arxiv>
      )}
      {item.data.date && (
        <PubDate>{dayjs(item.data.date).format('DD MMMM YYYY')}</PubDate>
      )}
    </Result>
  )
}
...
Publication.propTypes = {
  item: PropTypes.shape({
    data: PropTypes.shape({
      ...
      belongs_to_group: PropTypes.arrayOf(
        PropTypes.shape({
          sci_group: PropTypes.shape({
            id: PropTypes.string,
            data: PropTypes.shape({
              groupname: PropTypes.arrayOf(
                PropTypes.shape({
                  text: PropTypes.string,
                })
              ),
      ...
      eprint: PropTypes.arrayOf(
        PropTypes.shape({
          text: PropTypes.string,
        })
      ),
      journal_name: PropTypes.arrayOf(
        PropTypes.shape({
          text: PropTypes.string,
        })
      ),
