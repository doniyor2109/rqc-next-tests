...
export const fetchMainSciSliderRequest = () => ({
  type: actionTypes.FETCH_MAIN_SCI_SLIDER_REQUEST,
})

export const fetchMainSciSliderSuccess = response => ({
  type: actionTypes.FETCH_MAIN_SCI_SLIDER_SUCCESS,
  response,
})

export const fetchMainSciSliderFailure = error => ({
  type: actionTypes.FETCH_MAIN_SCI_SLIDER_FAILURE,
  error,
})

export const fetchMainSciSlider = language => dispatch => {
  dispatch(fetchMainSciSliderRequest())
  return Prismic.getApi(PrismicConfig.apiEndpoint).then(api => {
    api
      .query(Prismic.Predicates.at('document.type', 'scientist'), {
        lang: language,
